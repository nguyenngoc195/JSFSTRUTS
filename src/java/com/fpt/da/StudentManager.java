...
public class StudentManager {
    ...
    private PreparedStatement addStudentStatement() throws ClassNotFoundException, SQLException{
        if (searchByNameStatement == null) {
            ...
            searchByNameStatement = conn.prepareStatement("INSERT INTO SA.student (id, rollNumber, name, className, email) VALUES (?,?,?,?,?)");
        }
    ...
    private PreparedStatement getAllStatement() throws ClassNotFoundException, SQLException{
        if (searchByNameStatement == null) {
            Connection conn = DBConnection.getConnection();
            searchByNameStatement = conn.prepareStatement("SELECT * FROM SA.student");
        }
        return searchByNameStatement;
    }
